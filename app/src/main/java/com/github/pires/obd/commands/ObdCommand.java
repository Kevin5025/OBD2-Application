...
public abstract class ObdCommand {
    ...
    protected boolean isSetMaxNumberResponses = false;//Kevin5025
    private long start;
    private long end;
    ...
    /**
     * @author Kevin5025
     * @param maxNumberResponses the maximum number of (lines of) responses the obdii scanner should wait for before shortcircuiting its response time delay (default is 200 ms)
     * @return whether this operation did anything
     */
    public boolean setMaxNumberResponses(int maxNumberResponses) {
        if (maxNumberResponses < 1 || maxNumberResponses > 16) {
            return false;
        } else {
            unsetMaxNumberResponses();
            String maxNumberResponsesHexDigit = Integer.toString(maxNumberResponses, 16);
            cmd = cmd + maxNumberResponsesHexDigit;
            isSetMaxNumberResponses = true;
            return true;
        }
    }
     ...
     * @return whether this operation did anything
     */
    public boolean unsetMaxNumberResponses() {
        if (isSetMaxNumberResponses) {
            cmd = cmd.substring(0, cmd.length()-1);
            isSetMaxNumberResponses = false;
            return true;
        } else {
            return false;
        }
